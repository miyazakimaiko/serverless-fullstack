...
class MainStack extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    /** --------------------------------
     * tiktok-get-and-save-token lambda
     * --------------------------------*/

    const tiktokGetAndSaveTokenLambdaRole = new iam.Role(this, 'tiktok-get-and-save-token-lambda-role', {
      roleName: `${id}-tiktok-get-and-save-token-lambda-role`,
      assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com'),
    });

    tiktokGetAndSaveTokenLambdaRole.addToPolicy(new iam.PolicyStatement({
      actions: ['ssm:GetParameter'],
      resources: [tokenEncryptionKeySsm.parameterArn],
    }));

    // Lambdaを指定のVPCに配置する際にNetworkInterfaceが必要なため
    tiktokGetAndSaveTokenLambdaRole.addToPolicy(networkInterfaceActionPolicy);

    auroraCluster.grantConnect(tiktokGetAndSaveTokenLambdaRole, process.env.DB_USER);

    const tiktokGetAndSaveTokenHandler = new NodejsFunction(this, 'tiktok-get-and-save-token-lambda', {
      functionName: `${id}-tiktok-get-and-save-token`,
      handler: 'handler',
      entry: 'backend/functions/tiktok-get-and-save-token/index.js',
      runtime: lambda.Runtime.NODEJS_20_X,
      timeout: cdk.Duration.seconds(10),
      logRetention: logs.RetentionDays.ONE_WEEK,
      vpc, // DBと通信するためにDBと同じVPC内に配置
    ...
    tiktokPath.resourceForPath('token')
      .addMethod('POST', new apigateway.LambdaIntegration(tiktokGetAndSaveTokenHandler));
