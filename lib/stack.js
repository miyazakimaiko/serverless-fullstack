...
class SpaSampleSiteStack extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    const mediaBucket = new s3.Bucket(this, `${id}-media-bucket`, {
      ...
      removalPolicy: cdk.RemovalPolicy.DESTROY, // テストスタック用
      autoDeleteObjects: true, // テストスタック用
      cors: [ // フロントエンドから署名付きURLでのアップロードをを許可するため
      {
        allowedMethods: [
          s3.HttpMethods.GET,
          s3.HttpMethods.POST,
    ...
    // maiko  | dflskdf | https://126        | 2024-03-08 | 2024-03-10
    ...
    // S3のサイトを分配するCloudFront
    // const distribution = new cloudfront.Distribution(this, `${stage}CloudFrontDistribution`, {
    //   defaultBehavior: {
...
module.exports = { SpaSampleSiteStack }