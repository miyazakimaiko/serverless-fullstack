...
class MainStack extends cdk.Stack {
  constructor(scope, id, props) {
    ...
    /** ----------------------------------------
     *  insta-get-accounts lambda
     * ----------------------------------------*/

    const instaGetAccountsLambdaRole = new iam.Role(this, 'insta-get-accounts-lambda-role', {
      roleName: `${id}-insta-get-accounts-lambda-role`,
      assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com'),
    });

    instaGetAccountsLambdaRole.addToPolicy(new iam.PolicyStatement({
      actions: ['ssm:GetParameter'],
      resources: [tokenEncryptionKeySsm.parameterArn],
    }));

    // Lambdaを指定のVPCに配置する際にNetworkInterfaceが必要なため
    instaGetAccountsLambdaRole.addToPolicy(networkInterfaceActionPolicy);

    instaGetAccountsLambdaRole.addToPolicy(logPolicy);

    const instaGetAccountsLambda = new NodejsFunction(this, 'insta-get-accounts-lambda', {
      functionName: `${id}-insta-get-accounts`,
      handler: 'handler',
      entry: 'backend/functions/insta-get-accounts/index.js',
      runtime: lambda.Runtime.NODEJS_20_X,
      timeout: cdk.Duration.seconds(15),
      logRetention: logs.RetentionDays.ONE_WEEK,
      vpc, // NAT gateway 経由で外部と通信するため、NATのあるVPC内に配置
      vpcSubnets: {
        subnetType: ec2.SubnetType.PRIVATE_WITH_EGRESS,
      },
      environment: {
      ...
      role: instaGetAccountsLambdaRole,
    });
    ...
    instaPath.resourceForPath('long-lived-user-token')
      .addMethod('GET', new apigateway.LambdaIntegration(instaGetAccountsLambda));
